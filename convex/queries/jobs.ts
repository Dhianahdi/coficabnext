...
export const getJobs = query(async ({ db }) => {
  ...
  return await Promise.all(
    jobs.map(async (job) => {
      ...
      const recruiterImage = recruiter?.image || ""; // Fetch recruiter image
      const recruiterEmail = recruiter?.email || ""; // Fetch recruiter email
      ...
      return {
        ...
        recruiterImage, // Add recruiter image
        recruiterEmail, // Add recruiter email
...
export const getExperienceLevelOptions = query({
    args: {},
    handler: async ({ db }) => {
      return await db.query("experienceLevelOptions").collect();
    },
  });
  
  export const getEmploymentTypeOptions = query({
    args: {},
    handler: async ({ db }) => {
